"""
Module containing different downward options for FPN.
adapted from https://github.com/SwinTransformer/Video-Swin-Transformer
"""
...
class ResDown(nn.Module):
    ...
    def __init__(
        ...
        n_blocks = [3, 4, {"resnet50": 6, "resnet101": 23}[res_architecture], 3]
        ...
        start_filts_exp = start_filts * block_expansion
        ...
        C2_layers.append(block(start_filts, start_filts, conv=conv, stride=1, norm=norm, relu=relu,
                                    downsample=(start_filts, block_expansion, 1)))
        for _ in range(1, n_blocks[0]):
            C2_layers.append(block(start_filts_exp, start_filts, conv=conv, norm=norm, relu=relu))
        self.C2 = nn.Sequential(*C2_layers)

        C3_layers = []
        C3_layers.append(block(start_filts_exp, start_filts * 2, conv=conv, stride=2, norm=norm, relu=relu,
                                    downsample=(start_filts_exp, 2, 2)))
        for _ in range(1, n_blocks[1]):
            C3_layers.append(block(start_filts_exp * 2, start_filts * 2, conv=conv, norm=norm, relu=relu))
        self.C3 = nn.Sequential(*C3_layers)

        C4_layers = []
        C4_layers.append(block(
            start_filts_exp * 2, start_filts * 4, conv=conv, stride=2, norm=norm, relu=relu, downsample=(start_filts_exp * 2, 2, 2)))
        for _ in range(1, n_blocks[2]):
            C4_layers.append(block(start_filts_exp * 4, start_filts * 4, conv=conv, norm=norm, relu=relu))
        self.C4 = nn.Sequential(*C4_layers)

        C5_layers = []
        C5_layers.append(block(
            start_filts_exp * 4, start_filts * 8, conv=conv, stride=2, norm=norm, relu=relu, downsample=(start_filts_exp * 4, 2, 2)))
        for _ in range(1, n_blocks[3]):
            C5_layers.append(block(start_filts_exp * 8, start_filts * 8, conv=conv, norm=norm, relu=relu))
        self.C5 = nn.Sequential(*C5_layers)

        if self.sixth_pooling:
            C6_layers = []
            C6_layers.append(block(
...
class SwinDown(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed3D(
            operate_stride1=False, sixth_pooling=False, res_architecture='resnet50',
            norm=None, relu='relu', start_filts=24, n_channels=1
        )
...
class PatchEmbed3D(nn.Module):
    ...
    """
    def __init__(
        self, operate_stride1, sixth_pooling, res_architecture, norm, relu, start_filts, n_channels):
        super().__init__()
        self.operate_stride1 = operate_stride1
        self.sixth_pooling = sixth_pooling
        n_blocks = [3, 4, {"resnet50": 6, "resnet101": 23}[res_architecture], 3]
