...
class FPN(nn.Module):
    ...
    def __init__(
        ...
        if operate_stride1:
            ...
            self.C1 = conv(start_filts, start_filts, ks=7, stride=(2, 2, 2), pad=3, norm=norm, relu=relu)

        else:
            self.C1 = conv(n_channels, start_filts, ks=7, stride=(2, 2, 2), pad=3, norm=norm, relu=relu)
