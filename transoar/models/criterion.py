...
class TransoarCriterion(nn.Module):
    ...
    def forward(self, outputs, det_targets, seg_targets, anchors):
        ...
        matches, soft_labels = self.matcher(outputs, det_targets, anchors)

        # Compute the average number of target boxes accross all nodes, for normalization purposes
        num_boxes = sum(len(t["labels"]) for t in det_targets)

        # Compute losses
        loss_cls = self.loss_class(outputs, soft_labels)
        loss_bbox, loss_giou = self.loss_bboxes(outputs, det_targets, matches, num_boxes)
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                matches, soft_labels = self.matcher(aux_outputs, det_targets, anchors)

                loss_cls = self.loss_class(aux_outputs, soft_labels)
                loss_bbox, loss_giou = self.loss_bboxes(aux_outputs, det_targets, matches, num_boxes)
